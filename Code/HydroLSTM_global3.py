...
class Model_hydro_lstm_global(nn.Module):
    def __init__(self, input_size: int, lag: int, state_size: int, dropout: float, attributes: int ):
        super(Model_hydro_lstm_global, self).__init__()
        self.input_size = input_size #??input_size*lag
        self.state_size = state_size
        self.dropout = nn.Dropout(p=dropout)
        self.attributes_size = attributes
        self.h_t = torch.zeros(1)  # initialization of the previous output
        self.c_t = torch.zeros(1)  # initialization of the previous state
        self.hydro_lstm_global = HYDROLSTM_global(input_size, lag, state_size, attributes_size = attributes)
        self.regression = nn.Linear(state_size, 1, bias=True)
        #self.at_w = RF_weights #dataframe with catchmnet ID as index
    ...
    def forward(self, x, RF_weights, RF_regression):
        ...
        #print(RF_regression.loc[catch_ID][0])
        self.h_t, self.c_t = self.hydro_lstm_global(x, h_0, c_0, RF_weights)
...
class HYDROLSTM_global(nn.Module):
    def __init__(self, input_size: int, lag:int, state_size: int, attributes_size: int):
        super(HYDROLSTM_global, self).__init__()
        self.input_size = input_size
        self.lag = lag
        self.state_size = state_size
        self.attributes_size = attributes_size
        #self.weight_dataframe = weight_input_catch #dataframe with catchmnet ID as index
        self.weight_input = []
        self.bias = []
        self.catch_ID =[]
    ...
    def forward(self, x: torch.Tensor, h_0, c_0, RF_weights):
        ...
        if self.catch_ID != x[0,-1]:
            # Reset gradients
            #if self.catch_ID != []:
            #    self.weight_input.grad.grad.zero_()
            if self.state_size == 1:
        ...
        #print(self.weight_input.data.dtype)
        h_n = h_0
        c_n = c_0
        ...
        for it in range(batch_size):
            ...
            #h_1 = torch.mul(o, torch.relu(c_1))  # linear reservoir
